...
export class AdminComponent {
  ...
  comments: { target: string; comment: ViewComment, reply?: ViewComment, href: string }[] = [];
  ...
  loadComments(): void {
    this.commentService.getCommentsAdmin(this.form.value.adminPassword).subscribe({
      next: (rawComments: CommentAdminView[]) => {
        const allComments: { target: string; comment: ViewComment, reply?: ViewComment, href: string }[] = [];
        for (let mocIndex = 0; mocIndex < rawComments.length; mocIndex++) {
          ...
          for (let commentIndex = 0; commentIndex < mocComments.value.length; commentIndex++) {
            const reply = mocComments.value.find(comment => comment.commentId === mocComments.value[commentIndex].replyCommentId);
            allComments.push({
              target: mocComments.key,
              comment: mocComments.value[commentIndex],
              reply: reply,
              href: href
            });
          }
  ...
  deleteComment(commentId: number, target: string) {
    this.commentService.deleteCommentAdmin(target, commentId, this.form.value.adminPassword).subscribe({
      next: () => {
        const index =
          this.comments.findIndex(c =>
            c.target === target && c.comment.commentId === commentId);
