    ...
    private startRenderLoop() {
        ...
        const update = () => {
            if (this.clock.getElapsedTime() > this.MAX_FPS) {
                ...
                const top = rect.y - canvasrect.y;
                const left = rect.x - canvasrect.x;
                console.log(left, rect.height - top, rect.width, rect.height);
                this.renderer.setSize(rectParent.width, rectParent.height);

                this.renderer.setViewport(0, 0, canvasrect.width, canvasrect.height);
                this.renderer.setScissor(0, 0, canvasrect.width, canvasrect.height);
                ...
                for (let i = 0; i < this.partListScenes.length; i++) {
                    ...
                    // set the viewport
                    this.renderer.setViewport(rect.left, 0, rect.width, 96);
                    this.renderer.setScissor(rect.left, 0, rect.width, 96);
                    // this.renderer.clearColor();
                    // this.renderer.clearDepth();
                    this.renderer.render(scene, camera);
                }
