...
export class MocComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    this.route.paramMap.subscribe(paramMap => {
      ...
      this.metaService.setAllTags(this.moc.title + " - FlosRocketBricks", this.moc.mocDescription, "https://flosrocketbricks.com/moc/" + this.moc.id.toString() + "/" + this.moc.title.toLowerCase().replace("/","-").replace("'","-").replace(" ","-").replace(".","-"), "https://flosrocketbricks.com/" + this.moc.smallCoverImage);
